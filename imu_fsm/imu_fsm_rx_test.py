...
OUTPUT_FILE = "harry_run_long_2.csv"
...
async def collect(label, mode):
    ...
    async with BleakClient(device) as client:
        ...
        buffer = {"AX":0, "AY":0, "AZ":0, "GX":0, "GY":0, "GZ":0, "ASVM":0, "GSVM":0, "MCU_TIME":0, "FALL_EVENT":0}
        recv_buffer = ""
        ...
        def handle(sender, data):
            ...
            while True:
                ...
                try:
                    ax, ay, az, gx, gy, gz, asvm, gsvm, mcu_time, fall_event = map(float, parts[:10])
                    buffer["AX"] = ax
                    buffer["AY"] = ay
                    buffer["AZ"] = az
                    buffer["GX"] = gx
                ...
                try:
                    while True:
                        row = [
                            ...
                            buffer["MCU_TIME"], buffer["FALL_EVENT"],
                            label
                        ]
                        writer.writerow(row)
                        print(label, row)
